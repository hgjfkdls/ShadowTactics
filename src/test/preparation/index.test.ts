...
// Helper: select identities and return state in ROLL phase
function prepRollState(seed: number): GameState {
    const base = createInitialGameState();
    const s1 = applyAction(base, {
        type: 'SELECT_IDENTITY', playerId: 'p1',
        cardId: base.players['p1'].identityCards![0]
    });
    const s2 = applyAction(s1, {
        type: 'SELECT_IDENTITY', playerId: 'p2',
        cardId: s1.players['p2'].identityCards![0]
    });
    return { ...s2, rngSeed: seed };
}

{
    // Deterministic no-tie roll: seed 12345 produces no tie
    const s2 = prepRollState(12345);
...
{
    // Simular empate: p1 ya tiró, forzar su resultado, p2 tirará el mismo
    let s2 = prepRollState(42);
    s2 = { ...s2, diceRolls: { p1: 7 }, lastTieRoll: undefined } as any;
    // Forzar semilla para que p2 también saque 7
    s2 = { ...s2, rngSeed: 12345 } as any;
    const r2 = applyAction(s2, { type: 'ROLL_DICE', playerId: 'p2' });
    if (r2.diceRolls['p1'] === undefined) {
        // Empate detectado
        assert(r2.lastTieRoll !== undefined && typeof r2.lastTieRoll === 'number',
        ...
        // No hubo empate (p2 sacó diferente), verificar igual
        assert(r2.diceRolls['p1'] !== undefined && r2.diceRolls['p2'] !== undefined,
            'ROLL_DICE — ambos tiraron');
    }
}

{
    // Bajo (p1 < p2): forzar p1=3, p2 no tiró
    let s2 = prepRollState(42);
    s2 = { ...s2, diceRolls: { p1: 3 }, lastTieRoll: undefined } as any;
    s2 = { ...s2, rngSeed: 9999 } as any;
    const r2 = applyAction(s2, { type: 'ROLL_DICE', playerId: 'p2' });
    if (r2.preparationPhase !== 'ROLL') {
        assert(r2.diceRolls['p1']! < r2.diceRolls['p2']!,
            'low roll — p1 menor que p2');
    }
}

{
    // Alto (p1 > p2): forzar p1=10, p2 no tiró
    let s2 = prepRollState(42);
    s2 = { ...s2, diceRolls: { p1: 10 }, lastTieRoll: undefined } as any;
    s2 = { ...s2, rngSeed: 1 } as any;
    const r2 = applyAction(s2, { type: 'ROLL_DICE', playerId: 'p2' });
    if (r2.preparationPhase !== 'ROLL') {
        assert(r2.diceRolls['p1']! > r2.diceRolls['p2']!,
            'high roll — p1 mayor que p2');
    }
}

{
    // Empate: p1 ya tiró 7, forzar que p2 saque también 7
    let s2 = prepRollState(42);
    s2 = { ...s2, diceRolls: { p1: 7 }, lastTieRoll: undefined } as any;
    // Buscar semilla que dé 7 en p2 (probamos varias)
    let found = false;
    for (let seed = 0; seed < 1000; seed++) {
        const testState = { ...s2, rngSeed: seed } as any;
        const r = applyAction(testState, { type: 'ROLL_DICE', playerId: 'p2' });
        if (r.diceRolls['p1'] === undefined) {
            // Empate encontrado
            assert(r.lastTieRoll !== undefined, 'tie — lastTieRoll set');
            assertEqual(r.preparationPhase, 'ROLL', 'tie — sigue en ROLL');
            found = true;
            break;
        }
    }
    assert(found, 'tie — se encontró semilla que produce empate');
}
