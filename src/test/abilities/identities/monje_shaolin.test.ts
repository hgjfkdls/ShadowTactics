...

{
    ...
    s = {
        ...
        units: {
            ...s.units,
            gen1: { ...s.units['gen1'], abilities: ['meditacion'], hp: 15 },
        },
    };
    const st = applyAction(s, {
        type: 'USE_ABILITY', playerId: 'p1', unitId: 'gen1', abilityId: 'meditacion',
    });
    assert(st !== s, 'Meditación — cura 3 HP al general');
    assert(st.units['gen1']?.hp === 18, 'Meditación — general pasa de 15 a 18 HP');
}

{
    ...
    s = {
        ...
        units: {
            ...s.units,
            gen1: { ...s.units['gen1'], abilities: ['meditacion'], hp: 20 },
        },
    };
    const st = applyAction(s, {
        type: 'USE_ABILITY', playerId: 'p1', unitId: 'gen1', abilityId: 'meditacion',
    });
    assert(st !== s, 'Meditación — no rechaza a full HP (cura 0, consume PA igual)');
    assert(st.units['gen1']?.hp === 20, 'Meditación — HP no cambia cuando está a full');
}

{
    ...
    assert(st === s, 'Meditación — rechazado si PA < 2');
}
