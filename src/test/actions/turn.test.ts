...

{
    ...
    const moved = applyAction(state, { type: 'MOVE_UNIT', playerId: 'p1', unitId: 'u1', to: { q: 1, r: 0 } });
...

{
    ...
    const result = applyAction(setup, { type: 'ATTACK_UNIT', playerId: 'p1', unitId: 'u2', targetId: 'general-p2' });
...

{
    ...
    const u2 = state.units['u2'];
    const setup: GameState = { ...state, units: { ...state.units, 'general-p2': { ...general, position: { q: 3, r: 0 }, hp: 1 }, 'u2': { ...u2, difficulty: 2 } } };
    const afterKill = applyAction(setup, { type: 'ATTACK_UNIT', playerId: 'p1', unitId: 'u2', targetId: 'general-p2' });
    const secondAction = applyAction(afterKill, { type: 'MOVE_UNIT', playerId: 'p1', unitId: 'u1', to: { q: 1, r: 0 } });
