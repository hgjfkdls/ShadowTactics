...
import { getMoveRange } from '../board/movementRange';
...
import { useHexClick } from '../board/handlers/useHexClick';
...
export function HexBoard3D({ state, sendAction, mode = 'GAME', playerId, selectedDeployUnitId, selectedInfo, onInfoSelect, addAlert, disableInput, cameraMode = 'orthographic' }: Props) {
  ...
  const [unitAnimations, setUnitAnimations] = useState<Record<string, string | null>>({});
  const prevHistoryLengthRef = useRef(0);
  const attackTimeoutsRef = useRef<Record<string, ReturnType<typeof setTimeout>>>({});

  const ATTACK_ANIM_DURATION = 1200;
  ...
  useEffect(() => {
    ...
    const currentLength = state.gameHistory.length;

    if (currentLength > prevHistoryLengthRef.current) {
      const newEntries = state.gameHistory.slice(prevHistoryLengthRef.current);
      for (const entry of newEntries) {
        if (entry.type === 'attack') {
          const id = entry.attackerId;
          setUnitAnimations(prev => ({ ...prev, [id]: 'Attack' }));

          if (attackTimeoutsRef.current[id]) {
            clearTimeout(attackTimeoutsRef.current[id]);
          }
          attackTimeoutsRef.current[id] = setTimeout(() => {
            setUnitAnimations(prev => ({ ...prev, [id]: null }));
            delete attackTimeoutsRef.current[id];
  ...
  const hexOwnerMap = useMemo(() => {
    const map: Record<string, string> = {};
    for (const unit of Object.values(state.units)) {
      map[`${unit.position.q},${unit.position.r}`] = unit.owner;
    }
    return map;
  }, [state.units]);
      ...
      <BoardCanvas cameraMode={cameraMode}>
        ...
        {Object.values(state.units).map(unit => (
          <Unit3D
            ...
            hp={unit.hp}
            maxHp={10}
            position={animPositions?.[unit.id] ?? unit.position}
            selected={unit.id === selectedUnitId}
            hovered={hoveredUnitId === unit.id}
            animationName={unitAnimations[unit.id] ?? 'Idle'}
            onSelect={unitId => {
              if (isCardTargetMode) {
                const u = state.units[unitId];
                if (u && (isCardTargetAlly ? u.owner === myPlayerId : u.owner !== myPlayerId)) {
                  const cardId = selectedInfo?.cardId ?? '';
