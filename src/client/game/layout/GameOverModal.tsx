...
function reasonLabel(reason: string | undefined, isVictory: boolean): string {
    switch (reason) {
        case 'general_killed': return isVictory ? 'El general enemigo ha sido eliminado' : 'Tu general ha sido eliminado';
        case 'surrender': return isVictory ? 'El oponente se ha rendido' : 'Te has rendido';
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
            <div className="bg-zinc-900 border-2 border-zinc-700 rounded-xl px-10 py-8 shadow-2xl min-w-80 text-center space-y-5">
                ...
                <div className="text-xs text-zinc-500 space-y-1">
                    <div>Tiempo total: {formatTime(elapsed)}</div>
                    <div>Turnos: {state.turn}</div>
                </div>
