...
type Props = {
    ...
    onSelectDeployUnit?: (unitId: string) => void;
...
export function BottomPanel({ state, playerId, selectedUnitId, canAct, mode = 'GAME', sendAction, addAlert, onRequestMove, onRequestAttack, onRequestAbilityTarget, onAngelGuardian, onCardSelect, onSelectDeployUnit, selectedDeployUnitId }: Props) {
    ...
    // Deployment mode: show unit pool
    if (mode === 'DEPLOYMENT') {
        const pool = state.players[playerId]?.unitsToDeploy ?? [];
        const deployOrder = ['archer', 'infantry', 'cavalry', 'lancer', 'general'];
        const countByClass: Record<string, number> = {};
        const firstIds: Record<string, string> = {};
        for (const entry of pool) {
            countByClass[entry.unitClass] = (countByClass[entry.unitClass] ?? 0) + 1;
            if (!firstIds[entry.unitClass]) firstIds[entry.unitClass] = entry.unitId;
        }
        const isMyDeployTurn = state.currentDeployingPlayer === playerId;
        ...
        return (
            <div className="absolute bottom-0 left-0 right-0 z-30 flex justify-center pointer-events-none">
                <div className="bg-zinc-800/80 border border-zinc-600/60 border-b-0 rounded-t-lg shadow-xl pointer-events-auto flex flex-col h-[132px] min-h-[132px] w-[800px] max-w-[95vw]">
                    <div className="text-[9px] text-zinc-500 font-semibold uppercase tracking-wide text-center py-1 border-b border-zinc-700/40 shrink-0">{l('board.turnLabel')} {l('deploy.title')}</div>
                    <div className="flex-1 flex items-center justify-center gap-6 px-4">
                        {deployOrder.map(cls => {
                            const count = countByClass[cls] ?? 0;
                            const unitId = firstIds[cls];
                            const isSelected = selectedDeployUnitId === unitId;
                            const disabled = !isMyDeployTurn || count === 0;
                            return (
                                <button
                                    key={cls}
                                    disabled={disabled}
                                    onClick={() => unitId && onSelectDeployUnit?.(unitId)}
                                    className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-xl transition cursor-pointer ${
                                        isSelected
                                ...
                                >
                                    <img src={CLASS_ICONS[cls] || ''} alt={cls} className="w-10 h-10 object-contain" />
                                    <span className="text-[10px] font-bold text-zinc-300">{l(`unit.class.${cls}`)}</span>
